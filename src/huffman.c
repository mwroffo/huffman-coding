...
static void create_tree_nodes (Context *ctx) {
  ...
  // queue.
  for (int i=0; i<NUMBER_OF_CHARS; i++) {
    if ((ctx->table[i]).v > 0) { // if freq is NOT zero,
      TreeNode *T = tree_new();
      T->freq = ctx->table[i];
      T->type = LEAF;
      assert(T->left == NULL);
      assert(T->right == NULL);
      pqueue_enqueue(ctx->pq, T);
    }
  }
}
...
static TreeNode *build_tree (Context *ctx) {
  ...
  TreeNode *toReturn = pqueue_dequeue(ctx->pq);
  if (toReturn == NULL) printf("build_tree returning NULL!\n");
  return toReturn;
}
