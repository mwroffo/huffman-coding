...
char *table_bit_encode (EncodeTable *etab, unsigned char c) {
  ...
  // YEP: those are null pointers, all right.
  // printf("b =\t%p\n", b);
...
EncodeTable *table_build (TreeNode *root) {
  ...
  BitArray *new_bit_array = bit_array_new();
  // rec_gen_table is not loading etab->table... hmm... 
  rec_gen_table(etab, root, new_bit_array);

  // Return the constructed table:
  return etab; // returning a table of null BitArray pointers. why?
}
